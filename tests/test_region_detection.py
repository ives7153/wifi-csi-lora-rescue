...
def _raw_csi_json(node_id: int = 1) -> str:
...
def _feature_frame(node_id: int, level: int = 32) -> dict[str, object]:
    return {
...
def _ingest_cycle(detector: TriangleRegionDetector, *, level: int, now: float) -> None:
    for node_id in (1, 2, 3):
        detector.ingest(_feature_frame(node_id, level), now=now + node_id * 0.01)
